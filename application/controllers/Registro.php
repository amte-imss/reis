...
class Registro extends MY_Controller {
	...
	private function validarUsuarioRegistrado($usuario, $estado){
		...
		$exist = $this->mod_registro->getTaller(array('conditions'=>'T.usr_matricula=\''.$usuario.'\' AND YEAR(NOW())=YEAR(t_fecha_registro) AND t_estado='.$estado.' AND A.a_tipo=1'));
		switch ($estado) {
	...
	public function index($agenda = null)
	{
		...
		$sesiones_programadas = $this->mod_registro->getSesion(array('conditions'=>'a_estado = '.$estado_agenda['ACTIVO']['id'].' AND a_tipo=1', 'order'=>array('field'=>'a_inicio', 'type'=>'ASC'))); //buscamos las sesiones programadas
        $sesiones_programadas_distancia = $this->mod_registro->getSesion(array('conditions'=>'a_estado = '.$estado_agenda['ACTIVO']['id'].' AND a_tipo=2', 'order'=>array('field'=>'a_inicio', 'type'=>'ASC'))); //buscamos las sesiones programadas
...
//                echo "</pre>";
//                echo "Sesiones: <pre>";
//                print_r($sesiones_programadas_disponibles);
//                echo "</pre>";
//                echo "Estado agenda: <pre>";
//                print_r($estado_agenda['ACTIVO']['id']);
//                echo "</pre>";
...
//                $sesiones_programadas_distancia = $this->mod_registro->getSesion(array('conditions'=>'a_estado = '.$estado_agenda['ACTIVO']['id'].' AND YEAR(a_inicio)=YEAR(NOW()) and a_tipo=2 and agenda_id='.$agenda_id, 'order'=>array('field'=>'a_inicio', 'type'=>'ASC'))); //buscamos las sesiones programadas
                // No necesita los filtros que se aplican en las consultas anteriores (que están comentadas) pues para llegar a esta petición ya se ha filtrado antes
                $sesiones_programadas_distancia = $this->mod_registro->getSesion(array('conditions'=>' a_tipo=2 and agenda_id='.$agenda_id, ));
//		var_dump($sesiones_programadas_distancia);
	...
	public function registrosagenda()
	{
		...
		//$sesiones_programadas = $this->mod_registro->getSesion(array('conditions'=>'a_estado = '.$estado_agenda['ACTIVO']['id'].' AND YEAR(a_inicio)=YEAR(NOW()) and a_tipo=1', 'order'=>array('field'=>'a_inicio', 'type'=>'ASC'))); //buscamos las sesiones programadas
                 $sesiones_programadas_distancia = $this->mod_registro->getSesion(array( 'fields' => 'agenda_id, a_nombre, a_inicio, a_fin, a_registro, a_evaluacion_inicio, a_cupo, a_desc, a_estado, a_tipo, a_hr_inicio, a_hr_fin, a_registro_fin, a_duracion, a_liga, id_conferencia, texto_liga, IF(  NOW() > a_registro  AND NOW() < a_registro_fin , "1", "0") AS ontime ', // Se agrega el campo "ontime" que se calcaula con las fechas de iniciode inscripción y de inicio curso
                                                                                        'conditions'=>'a_estado = '.$estado_agenda['ACTIVO']['id'].' AND a_tipo=2', 'order'=>array('field'=>'a_inicio', 'type'=>'ASC'))); //buscamos las sesiones programadas
